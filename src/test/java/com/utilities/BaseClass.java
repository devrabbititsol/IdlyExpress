...
public class BaseClass {
	...
	public void printValidateLogAndReport(Logger logger, String data) throws Exception {
		logger.info(data);
		if(data.startsWith("Pass:")) {
			data = data.replace("Pass:", "");
			printSuccessLogAndReport(logger, data);
		} else {
			data = data.replace("Fail:", "");
			printFailureLogAndReport(logger, data);
		}
	}
